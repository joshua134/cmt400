        ...
        if(empty($newTitle) || empty($newContent)){
            ...
            if( $old_Depart_ID == $newDepartment ){
                ...
                if(empty($newImage['name'])){
                    ...
                    $sql = "UPDATE board SET title = '".$newTitle."', content='".$newContent."', updated_at='".$now."', departmentID='".$newDepartment."' WHERE id = '".$id."' AND is_notice = true";
                    if(mysqli_query($connection, $sql)){
                        echo "<script>alert('Notice updated successfully.');</script>";
                    }else{
                        echo "Error: ". mysqli_error($connection);
                    ...
                    if(!in_array($newImage_ext, $allowed_extensions)){
                        ...
                        $sql = "UPDATE board SET title = '".$newTitle."', content='".$newContent."', media='".$newImage['name']."', departmentID='".$newDepartment."',updated_at='".$now."' WHERE id = '".$id."' AND is_notice = true";
                        if(mysqli_query($connection, $sql)){
...
<body>
    <div class="header">
        <a href="dashboard.php">Home</a> / <a href="update-notice.php">Update Notice</a> / <a href="logout.php">Logout</a>
    </div>
    <div class="container">
        <div class="wrapper">
            ...
            <form method="post" action="update-notice.php" enctype="multipart/form-data"  onsubmit="return noticeUpdate()">
                ...
                <select id="department" name="department" style="margin:6px 0;">
                    ...
                    <option value="null">Notice for all students.</option>
                    <?php 
