<?php
    ...
    if( isset($_GET['fid']) && isset($_GET['bid']) ){
        ...
        if(mysqli_num_rows($result)>0){
            if(mysqli_query($connection, "UPDATE feedback set is_blocked=true WHERE id = $fID AND boardID=$bID")){
                echo "<script>alert('Feedback blocked successfully. \nPublic cannot see this  feedback.');</script>";
